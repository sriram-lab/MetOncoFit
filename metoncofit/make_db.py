...
for fil in os.listdir('./../data/median/'):
    ...
    for t in targ:
        ...
        if ("S. Table "+str(x+9)+" | "+canc) in book:
            final_df.to_excel(writer, sheet_name=("S. Table "+str(x+9)+" | "+canc), startrow=writer.sheets[("S. Table "+str(x+9)+" | "+canc)].max_row, header=False)
        else:
            final_df.to_excel(writer, sheet_name=("S. Table "+str(x+9)+" | "+canc), header=True)
        writer.save()
